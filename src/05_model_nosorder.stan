...
/*  eff_ep_block[1]=0;
  ...
  for (i in 1:N) {
    for( j in 1:nblocks){
      ep[i,j]  = Phi_approx( mu_p[1] + eff_ep_tacs[tacs[i,j]] + sigma[1] * ep_pr[i] );
      beta[i,j]= exp( mu_p[2] + eff_beta_tacs[tacs[i,j]] + sigma[2] * beta_pr[i] );
      pi[i,j]  = mu_p[3] + eff_pi_tacs[tacs[i,j]] + sigma[3] * pi_pr[i];
      b[i,j]   = mu_p[4] + eff_b_tacs[tacs[i,j]] + sigma[4] * b_pr[i];
    }
  }
}
model {  
